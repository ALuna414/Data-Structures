...
/* pseudocode:
     Going to create new reqNode
     get address of first and point to it
     set pointer next to next address
     previous pointer to null
     T o is going to point to address of reqNode
     Repeat for all initial requests
...
/*
 psuedocode:
     check address of reNode in terms of timestamp first
     start clock
     if two timestamo are same then check status
     while the one is greater then it answers
     add to clock
     check previous timestamp people in queue
     check for new reqNode person coming in
     if status is higher on new person than queue him next
     run clock until all calls answered.
...
/*
 pseudocode:
     if agent equal o or not busy/ false
     answer phone from calling InsertRequests
     initiate time variable and decrement
     if still busy then output he is busy
     else answer next perosn InsertRequests call
     if no calls left or not busy then null
     else true and end program
